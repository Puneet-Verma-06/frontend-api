...
export default function OtpModal({ regData, onClose, onBack, onSuccess }) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-[1000]" onClick={onClose}>
      <div className="bg-white rounded-xl shadow-xl w-md p-6 relative" onClick={(e) => e.stopPropagation()}>
        <button className="absolute cursor-pointer top-3 right-3 hover:opacity-80" style={{ color: GOLD }} onClick={onClose}>✕</button>

        <h3 className="text-lg font-semibold mb-1" style={{ color: GOLD }}>Verify Email</h3>
        <p className="text-sm text-gray-500 mb-4">
          Enter the 6-digit OTP sent to <span className="font-medium">{regData?.email}</span>.
        </p>

        {err && <div className="mb-3 text-sm bg-red-50 text-red-700 border border-red-200 rounded px-3 py-2">{err}</div>}
        {info && <div className="mb-3 text-sm bg-green-50 text-green-700 border border-green-200 rounded px-3 py-2">{info}</div>}
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="flex items-center justify-between">
            <button type="button" onClick={onBack} className="underline" style={{ color: GOLD }}>
              Back
            </button>
            <button type="button" onClick={resend} disabled={loading} className="underline" style={{ color: GOLD }}>
              Resend OTP
            </button>
          </div>
